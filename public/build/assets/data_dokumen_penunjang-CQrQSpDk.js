$(document).ready(function(){let i=[],r=1,l=3;u(1),typeof Swal>"u"&&document.write('<script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"><\/script>'),window.location.hash==="#dokumen-penunjang-section"&&setTimeout(function(){k()},300),$(document).on("click",".pagination-link",function(e){e.preventDefault(),r=$(this).data("page"),u(r),$("html, body").animate({scrollTop:$("#tableDokumenPenunjang").offset().top-100},500)});function k(){const e=$("#dokumen-penunjang-section");e.length&&$("html, body").animate({scrollTop:e.offset().top-80},500)}function c(e,n,a){const t=a>0?(e-1)*n+1:0,o=Math.min(e*n,a);$("#dokumenPaginationInfo").html(`Showing ${t} to ${o} of ${a} entries`)}$("#searchDokumenForm").on("submit",function(e){e.preventDefault();const n=$("#searchDokumenPenunjang").val(),a=new URL(window.location.href);a.searchParams.set("searchDokumenPenunjang",n),a.hash="dokumen-penunjang-section",window.history.pushState({},"",a.toString()),r=1,u(r),k()}),$("#btnTambahDokumen").on("click",function(){const e=$("#nama_dokumen_penunjang").val(),n=$("#jenis_dokumen_penunjang_id").val(),a=$("#jenis_dokumen_penunjang_id option:selected").text(),t=$("#file_dokumen_penunjang")[0];if(!e||!n||t.files.length===0){typeof Swal<"u"?Swal.fire({icon:"error",title:"Formulir Tidak Lengkap",text:"Silakan lengkapi semua field formulir"}):alert("Silakan lengkapi semua field formulir");return}const o={id:Date.now(),nama_dokumen_penunjang:e,jenis_dokumen_penunjang_id:n,jenis_dokumen:a,file:t.files[0],fileName:t.files[0].name};i.push(o),g(),$("#nama_dokumen_penunjang").val(""),$("#jenis_dokumen_penunjang_id").val(""),$("#file_dokumen_penunjang").val(""),i.length===1&&($("#previewDokumenSection").removeClass("d-none"),$("html, body").animate({scrollTop:$("#previewDokumenSection").offset().top-100},500))});function g(){const e=$("#previewDokumenTable tbody");e.empty(),i.forEach((n,a)=>{e.append(`
                <tr>
                    <td>${a+1}</td>
            ...
            `)}),$(".btn-preview-delete").on("click",function(){const n=$(this).data("id");f(n)})}function f(e){i=i.filter(n=>n.id!==e),i.length===0?$("#previewDokumenSection").addClass("d-none"):g()}$("#btnBatalPreview").on("click",function(){i=[],$("#previewDokumenSection").addClass("d-none")}),$("#btnSimpanDokumen").on("click",function(){if(i.length===0){Swal.fire({icon:"error",title:"Tidak Ada Dokumen",text:"Tidak ada dokumen untuk disimpan"});return}let e=0,n=0;const a=i.length;Swal.fire({title:"Menyimpan Dokumen...",html:`Menyimpan 0 dari ${a}`,allowOutsideClick:!1,didOpen:()=>{Swal.showLoading()}});const t=o=>{if(o>=i.length){n===0?Swal.fire({icon:"success",title:"Berhasil",text:`${e} dokumen berhasil disimpan`}):Swal.fire({icon:"warning",title:"Selesai Dengan Peringatan",html:`${e} dokumen berhasil disimpan<br>${n} dokumen gagal disimpan`}),i=[],$("#previewDokumenSection").addClass("d-none"),setTimeout(function(){r=1,u(r)},500);return}const m=i[o],s=new FormData;s.append("proyek_id",$('input[name="proyek_id"]').val()),s.append("nama_dokumen_penunjang",m.nama_dokumen_penunjang),s.append("jenis_dokumen_penunjang_id",m.jenis_dokumen_penunjang_id),s.append("file_dokumen_penunjang",m.file),s.append("_token",$('meta[name="csrf-token"]').attr("content")),Swal.update({html:`Menyimpan ${o+1} dari ${a}`}),$.ajax({url:"/profesional/proyek/dokumen-penunjang",type:"POST",data:s,processData:!1,contentType:!1,headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")},success:function(d){d.success?e++:(n++,console.error("Error saving document:",d.message)),t(o+1)},error:function(d){n++,console.error("Error saving document:",d.responseText),t(o+1)}})};t(0)});function u(e=1){const n=$('input[name="proyek_id"]').val(),a=$("#searchDokumenPenunjang").val()||"";$("#tableDokumenPenunjang tbody").html(`
            <tr>
        ...
        `),$("#dokumenPagination").html(""),$.ajax({url:`/profesional/proyek/${n}/dokumen-penunjang`,type:"GET",data:{search:a,page:e,per_page:l},headers:{"X-CSRF-TOKEN":$('meta[name="csrf-token"]').attr("content")},success:function(t){if(t&&t.success&&t.data)if(t.data.total!==void 0?c(t.data.current_page||e,t.data.per_page||l,t.data.total||0):t.pagination&&c(t.pagination.current_page||e,t.pagination.per_page||l,t.pagination.total||0),t.data.data){const o=t.data.data;o&&o.length>0?(h(o),t.pagination&&t.pagination.html?$("#dokumenPagination").html(t.pagination.html):$("#dokumenPagination").html("")):(p(),$("#dokumenPagination").html(""))}else Array.isArray(t.data)&&t.data.length>0?h(t.data):p();else p(),$("#dokumenPagination").html(""),c(1,l,0)},error:function(t){console.error("Error loading dokumen:",t.responseText),$("#tableDokumenPenunjang tbody").html(`
                    <tr>
                ...
                `),$("#dokumenPagination").html(""),c(1,l,0)}})}function h(e){const n=$("#tableDokumenPenunjang tbody");if(n.empty(),!e||!Array.isArray(e)||e.length===0){p();return}$("#emptyDokumenMessage").addClass("d-none"),e.forEach((a,t)=>{if(!a||typeof a!="object"){console.error("Invalid dokumen data:",a);return}const o=a.dokumen_penunjang_proyek_id,m=a.nama_dokumen_penunjang||"Tidak ada nama",s=a.jenis_dokumen||"Tidak diketahui",d=a.created_at;n.append(`
                <tr>
                    <td>${m}</td>
                    <td>${s}</td>
                    <td>${w(d)}</td>
                    <td>
                        <div class="d-flex gap-2">
                            <a href="/profesional/proyek/dokumen-penunjang/download/${o}" 
            ...
            `)}),v()}function w(e){if(!e)return"-";const n=new Date(e);return isNaN(n.getTime())?e:n.toLocaleDateString("id-ID",{day:"2-digit",month:"long",year:"numeric"})}function p(){$("#tableDokumenPenunjang tbody").empty(),$("#emptyDokumenMessage").removeClass("d-none");const e=$("#searchDokumenPenunjang").val()||"";e?$("#emptyDokumenMessage div").html(`
