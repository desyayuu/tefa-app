...
class DataKeluarKeuanganProyekDosenController extends Controller
{
     ...
     * Memastikan dosen yang login adalah project leader dari proyek tersebut
     */
    private function verifyProjectAccess($proyekId)
    {
        $dosenId = $this->getDosenId();
        
        // Handle jika getDosenId() return response (error case)
        if ($dosenId instanceof \Illuminate\Http\JsonResponse) {
            return false;
        }
     ...
     * Memastikan transaksi berada dalam proyek yang dipimpin dosen login
     */
    ...
    public function getDataProyek(Request $request)
    {
        ...
        return view('pages.Dosen.DataKeuangan.table_proyek_dana_keluar', [
            'proyek' => $proyek,
            'search' => $search,
    ...
    public function downloadBuktiTransaksi($fileName)
    {
        ...
        // Verify file belongs to a transaction in project led by current dosen
        $hasAccess = DB::table('t_keuangan_tefa')
            ->join('m_proyek', 't_keuangan_tefa.proyek_id', '=', 'm_proyek.proyek_id')
            ->join('t_project_leader', 'm_proyek.proyek_id', '=', 't_project_leader.proyek_id')
    ...
    public function getDataKeluarKeuanganProyek(Request $request, $proyekId)
    {
        // ✅ VERIFIKASI HAK AKSES PROYEK
        if (!$this->verifyProjectAccess($proyekId)) {
            return redirect()->route('dosen.dataKeluarKeuanganProyek')
        ...
        return view('pages.Dosen.DataKeuangan.kelola_dana_keluar_proyek', [
            'proyek' => $proyek,
