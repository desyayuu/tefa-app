...
class DataKeluarKeuanganProyekProfesionalController extends Controller
{
    ...
    /**
     * ✅ TAMBAHAN: Helper method untuk memverifikasi hak akses proyek
     * Memastikan profesional yang login adalah project leader dari proyek tersebut
     */
    private function verifyProjectAccess($proyekId)
    {
        $profesionalId = $this->getProfesionalId();
        
        // Handle jika getProfesionalId() return response (error case)
        if ($profesionalId instanceof \Illuminate\Http\JsonResponse) {
            return false;
        }

        $hasAccess = DB::table('m_proyek')
            ->join('t_project_leader', 'm_proyek.proyek_id', '=', 't_project_leader.proyek_id')
        ...
        Log::info('Project access verification', [
            'proyek_id' => $proyekId,
    ...
    /**
     * ✅ TAMBAHAN: Helper method untuk memverifikasi hak akses transaksi
     * Memastikan transaksi berada dalam proyek yang dipimpin profesional login
     */
    private function verifyTransactionAccess($transaksiId)
    {
        $profesionalId = $this->getProfesionalId();
        ...
        $hasAccess = DB::table('t_keuangan_tefa')
        ...
        Log::info('Transaction access verification', [
    ...
    /**
     * ✅ TAMBAHAN: Helper method untuk response unauthorized
     */
    private function unauthorizedResponse($message = 'Anda tidak memiliki akses ke data ini')
    {
    ...
    public function getDataProyek(Request $request)
    {
        ...
        $proyek = $query->orderByRaw("FIELD (m_proyek.status_proyek, 'In Progress', 'Initiation', 'Done')")
                        ->paginate(10);
                        
        return view('pages.Profesional.DataKeuangan.table_proyek_dana_keluar', [
            'proyek' => $proyek,
            'search' => $search,
    ...
    {
        try {
            // ✅ VERIFIKASI HAK AKSES PROYEK
            if (!$this->verifyProjectAccess($proyekId)) {
                return $this->unauthorizedResponse('Anda tidak memiliki akses ke proyek ini');
            }
    ...
    public function hapusTransaksi(Request $request, $transaksiId)
    {
        ...
        try {
            // ✅ VERIFIKASI HAK AKSES TRANSAKSI
            if (!$this->verifyTransactionAccess($transaksiId)) {
                DB::rollBack();
                return $this->unauthorizedResponse('Anda tidak memiliki akses untuk menghapus transaksi ini');
            }
    ...
    {
        // ✅ VERIFIKASI KEPEMILIKAN FILE MELALUI TRANSAKSI
        $profesionalId = $this->getProfesionalId();
        ...
        // Verify file belongs to a transaction in project led by current profesional
        $hasAccess = DB::table('t_keuangan_tefa')
            ->join('m_proyek', 't_keuangan_tefa.proyek_id', '=', 'm_proyek.proyek_id')
            ->join('t_project_leader', 'm_proyek.proyek_id', '=', 't_project_leader.proyek_id')
            ->where('t_keuangan_tefa.bukti_transaksi', 'uploads/keuangan_tefa/' . $fileName)
            ->where('t_project_leader.leader_type', 'Profesional')
            ->where('t_project_leader.leader_id', $profesionalId)
            ->whereNull('t_keuangan_tefa.deleted_at')
            ->whereNull('m_proyek.deleted_at')
        ...
        $path = public_path('uploads/keuangan_tefa/' . $fileName);
    ...
    public function getDataKeluarKeuanganProyek(Request $request, $proyekId)
    {
        // ✅ VERIFIKASI HAK AKSES PROYEK
        if (!$this->verifyProjectAccess($proyekId)) {
            return redirect()->route('profesional.dataKeluarKeuanganProyek')
        ...
        return view('pages.Profesional.DataKeuangan.kelola_dana_keluar_proyek', [
            'proyek' => $proyek,
            'jenisTransaksi' => $jenisTransaksi,
            'jenisKeuanganTefa' => $jenisKeuanganTefa,
            'subkategoriPengeluaran' => $subkategoriPengeluaran,
            'hasSubkategoriPengeluaran' => $subkategoriPengeluaran->count() > 0,
    ...
    public function tambahTransaksiPengeluaran(Request $request)
    {
        try {
            ...
            if (!$proyekId || !$this->verifyProjectAccess($proyekId)) {
                return $this->unauthorizedResponse('Anda tidak memiliki akses untuk menambah transaksi pada proyek ini');
            }
            ...
            $proyekKategoriId = DB::table('m_jenis_keuangan_tefa')
                ->where('nama_jenis_keuangan_tefa', 'Proyek')
                ->value('jenis_keuangan_tefa_id');
            ...
            $request->merge([
                ...
                'jenis_keuangan_tefa_id' => $proyekKategoriId
            ]);
        ...
        try {
            ...
            foreach ($pengeluaranData as $item) {
                ...
                $data = [
                    ...
                    'tanggal_transaksi' => date('Y-m-d', strtotime($item['tanggal_transaksi'])),
                    'jenis_transaksi_id' => $pengeluaranId,
                    'jenis_keuangan_tefa_id' => $proyekKategoriId,
    ...
    public function getTransaksiDetail(Request $request, $transaksiId)
    {
        try {
            // ✅ VERIFIKASI HAK AKSES TRANSAKSI
            if (!$this->verifyTransactionAccess($transaksiId)) {
                return $this->unauthorizedResponse('Anda tidak memiliki akses ke transaksi ini');
            }
    ...
    public function updateTransaksiPengeluaran(Request $request, $transaksiId)
    {
        try {
            // ✅ VERIFIKASI HAK AKSES TRANSAKSI
            if (!$this->verifyTransactionAccess($transaksiId)) {
                return $this->unauthorizedResponse('Anda tidak memiliki akses untuk mengupdate transaksi ini');
            }
            ...
            // Check if subkategori is required based on data availability
            ...
            // Handle subkategori sesuai ketersediaan dan requirement
            $subkategoriId = null;
            if ($isSubkategoriRequired) {
    ...
    private function isSubkategoriRequiredForPengeluaran()
    {
        try {
            ...
            // Check if subcategories exist - jika ada data maka REQUIRED
