...
class DataMasukKeuanganProyekController extends Controller{
    ...
    public function getTransaksiDetailForEdit(Request $request, $proyekId, $transaksiId)
    {
        try {
            ...
            $transaksi = DB::table('t_keuangan_tefa')
                ...
                ->where('m_jenis_transaksi.nama_jenis_transaksi', 'Pemasukan') // Only allow income transactions
    ...
    public function getDataMasukKeuanganProyek(Request $request, $proyekId)
    {
        ...
        $jenisTransaksi = DB::table('m_jenis_transaksi')
            ->where('nama_jenis_transaksi', 'Pemasukan')
            ->whereNull('deleted_at')
            ->get();
        ...
        // Get subkategori pemasukan - ENHANCED to handle empty cases better
        $subkategoriPemasukan = $this->getSubkategoriPemasukanForProyek();
        
        // Enhanced logging for debugging
        Log::info('Subkategori pemasukan for proyek', [
            'proyek_id' => $proyekId,
            'subkategori_count' => $subkategoriPemasukan->count(),
            'has_subkategori' => $subkategoriPemasukan->count() > 0
        ]);
        ...
        return view('pages.Koordinator.DataKeuanganProyek.kelola_dana_masuk_proyek', [
            ...
            'subkategoriPemasukan' => $subkategoriPemasukan,
            'hasSubkategoriPemasukan' => $subkategoriPemasukan->count() > 0, // Add this flag
            'totalPemasukan' => $totalPemasukan,
            'titleSidebar' => 'Detail Pemasukan Keuangan Proyek'
        ]);
    }
    
    private function getSubkategoriPemasukanForProyek()
    {
        try {
            // Get "Pemasukan" transaction type ID
            ...
            Log::error('Error getting subkategori pemasukan for proyek: ' . $e->getMessage(), [
                'trace' => $e->getTraceAsString()
            ]);
            return collect([]);
        }
    ...
    {
        try {
            Log::info('Tambah transaksi pemasukan request:', [
                'is_single' => $request->input('is_single'),
                'tanggal_transaksi' => $request->input('tanggal_transaksi'),
                'has_file' => $request->hasFile('bukti_transaksi'),
                'has_subkategori' => $request->filled('subkategori_pemasukan_id'),
                'subkategori_value' => $request->input('subkategori_pemasukan_id'),
                'data_length' => $request->input('pemasukan_data') ? strlen($request->input('pemasukan_data')) : 0
            ]);
            
            // Get the IDs for "Pemasukan" and "Proyek"
            $pemasukanId = DB::table('m_jenis_transaksi')
                ->where('nama_jenis_transaksi', 'Pemasukan')
                ->value('jenis_transaksi_id');
            ...
            $request->merge([
                'jenis_transaksi_id' => $pemasukanId,
            ...
            if (!$isSingle) {
                // Multiple mode
                $jsonData = $request->input('pemasukan_data');
                if ($jsonData) {
                    $pemasukanData = json_decode($jsonData, true);
                    
                    if (!empty($pemasukanData)) {
            ...
            $validationRules = [
                ...
                'subkategori_pemasukan_id' => 'nullable|uuid',
            ];
            
            // Only add exists validation if subkategori is provided
            if ($request->filled('subkategori_pemasukan_id')) {
                $validationRules['subkategori_pemasukan_id'] = 'nullable|uuid|exists:m_sub_jenis_transaksi,sub_jenis_transaksi_id';
            }
            
            $validator = Validator::make($request->all(), $validationRules);
            
            if ($validator->fails()) {
                Log::warning('Validation failed for tambah transaksi pemasukan', [
            ...
            $subkategoriId = $request->filled('subkategori_pemasukan_id') ? 
                            $request->input('subkategori_pemasukan_id') : null;
    ...
    public function storeWithFiles(Request $request)
    {
        try {
            ...
            if (empty($jsonData)) {
                Log::error('No pemasukan_data received in request');
                throw new \Exception('Tidak ada data pemasukan untuk disimpan');
            }
            ...
            if (empty($pemasukanData)) {
                ...
                throw new \Exception('Data pemasukan tidak valid atau kosong');
            }
            
            Log::info('Multiple pemasukan data to process:', [
                'count' => count($pemasukanData),
                'sample_item' => isset($pemasukanData[0]) ? [
                    'has_subkategori' => !empty($pemasukanData[0]['subkategori_pemasukan_id']),
                    'subkategori_id' => $pemasukanData[0]['subkategori_pemasukan_id'] ?? 'null'
                ] : null
            ]);
            
            // Sort the items by sequence to maintain the order they were added in the form
            usort($pemasukanData, function($a, $b) {
            ...
            foreach ($pemasukanData as $item) {
                ...
                $subkategoriId = !empty($item['subkategori_pemasukan_id']) ? 
                                $item['subkategori_pemasukan_id'] : null;
            ...
            Log::info('Successfully stored multiple pemasukan records', [
            ...
            DB::rollBack();
            Log::error('Error storing pemasukan with files: ' . $e->getMessage(), [
                'trace' => $e->getTraceAsString()
            ]);
    ...
    public function updateTransaksiPemasukan(Request $request, $transaksiId)
    {
        try {
            Log::info('Update transaksi pemasukan request:', [
                'transaksi_id' => $transaksiId,
            ...
            // Get the default IDs
            $pemasukanRecord = DB::table('m_jenis_transaksi')
                ->where('nama_jenis_transaksi', 'Pemasukan')
                ->whereNull('deleted_at')
                ->first();
            ...
            if (!$pemasukanRecord || !$proyekRecord) {
                return response()->json([
                    'success' => false,
            ...
            $validator = Validator::make($request->all(), $validationRules, [
                'edit_sub_jenis_transaksi_id.required' => 'Kategori Pemasukan wajib dipilih karena tersedia dalam sistem.',
                'edit_sub_jenis_transaksi_id.exists' => 'Kategori Pemasukan yang dipilih tidak valid.',
            ]);

            if ($validator->fails()) {
                Log::warning('Validation failed for update transaksi pemasukan', [
                    'errors' => $validator->errors()->toArray(),
                    'transaksi_id' => $transaksiId,
    ...
    private function isSubkategoriRequiredForPemasukan()
    {
        try {
            // Get "Pemasukan" transaction type ID
            $jenisTransaksiId = DB::table('m_jenis_transaksi')
    ...
    public function getDataTransaksiProyek(Request $request, $proyekId)
    {
        ...
        $filters = [
            ...
            'kategori_pemasukan' => $request->get('kategori_pemasukan'),
        ...
        if (!empty($filters['kategori_pemasukan'])) {
            $query->where('t_keuangan_tefa.sub_jenis_transaksi_id', $filters['kategori_pemasukan']);
        }
    ...
    public function getKategoriPemasukanForFilter(Request $request)
    {
        try {
            // Get "Pemasukan" transaction type ID
            $jenisTransaksiId = DB::table('m_jenis_transaksi')
    ...
    public function getSummary(Request $request, $proyekId = null)
    {
        try {
            ...
            $pemasukanByKategori = DB::table('t_keuangan_tefa as kt')
                ->select(
            ...
            Log::info('Summary data retrieved successfully', [
                ...
                'kategori_count' => $pemasukanByKategori->count()
            ]);
