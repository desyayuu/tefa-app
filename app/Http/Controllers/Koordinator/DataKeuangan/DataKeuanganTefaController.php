...
class DataKeuanganTefaController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Get jenis transaksi and jenis keuangan first for conditional validation
            $jenisTransaksiId = $request->input('jenis_transaksi_id');
            $jenisKeuanganId = $request->input('jenis_keuangan_tefa_id');
            
            $jenisTransaksi = null;
            $jenisKeuanganTefa = null;
            ...
            // Add conditional validation for proyek
            if ($jenisKeuanganTefa && $jenisKeuanganTefa->nama_jenis_keuangan_tefa === 'Proyek') {
                $rules['proyek_id_selected'] = 'required|exists:m_proyek,proyek_id';
            } else {
                $rules['proyek_id_selected'] = 'nullable|exists:m_proyek,proyek_id';
            }
            
            // UPDATED: Add conditional validation for sub jenis transaksi dengan cek ketersediaan data
            if ($jenisTransaksi && ($jenisTransaksi->nama_jenis_transaksi === 'Pengeluaran' || $jenisTransaksi->nama_jenis_transaksi === 'Pemasukan')) {
                // Cek apakah ada sub jenis transaksi untuk kombinasi ini
                $hasSubJenisTransaksi = DB::table('m_sub_jenis_transaksi')
                    ->where('jenis_transaksi_id', $jenisTransaksiId)
                    ->where('jenis_keuangan_tefa_id', $jenisKeuanganId)
                    ->whereNull('deleted_at')
                    ->exists();
                
                if ($hasSubJenisTransaksi) {
                    // Ada data sub jenis transaksi - required
                    $rules['sub_jenis_transaksi_id'] = 'required|exists:m_sub_jenis_transaksi,sub_jenis_transaksi_id';
                    \Log::info('Sub jenis transaksi validation: REQUIRED (data exists)');
                } else {
                    // Tidak ada data sub jenis transaksi - nullable
                    $rules['sub_jenis_transaksi_id'] = 'nullable|exists:m_sub_jenis_transaksi,sub_jenis_transaksi_id';
                    \Log::info('Sub jenis transaksi validation: NULLABLE (no data available)');
                }
            } else {
                $rules['sub_jenis_transaksi_id'] = 'nullable|exists:m_sub_jenis_transaksi,sub_jenis_transaksi_id';
            }
            ...
            if ($isSingle) {
                ...
                // Mode multiple - simpan banyak data
                $keuanganTefaData = $this->processMultipleKeuanganTefa($request);
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            // UPDATED: Validasi untuk sub jenis transaksi dengan cek ketersediaan data
            if ($jenisTransaksi && ($jenisTransaksi->nama_jenis_transaksi === 'Pengeluaran' || $jenisTransaksi->nama_jenis_transaksi === 'Pemasukan')) {
                // Cek apakah ada sub jenis transaksi untuk kombinasi ini
                $hasSubJenisTransaksi = DB::table('m_sub_jenis_transaksi')
                    ->where('jenis_transaksi_id', $jenisTransaksiId)
                ...
                if ($hasSubJenisTransaksi) {
                    ...
                    \Log::info('Edit - Sub jenis transaksi validation: REQUIRED (data exists)');
                } else {
                    // Tidak ada data sub jenis transaksi - nullable
                    $rules['edit_sub_jenis_transaksi_id'] = 'nullable|exists:m_sub_jenis_transaksi,sub_jenis_transaksi_id';
                    \Log::info('Edit - Sub jenis transaksi validation: NULLABLE (no data available)');
                }
            } else {
                $rules['edit_sub_jenis_transaksi_id'] = 'nullable|exists:m_sub_jenis_transaksi,sub_jenis_transaksi_id';
            }
            ...
            // UPDATED: Set sub_jenis_transaksi_id dengan handling nullable
            if ($jenisTransaksi && ($jenisTransaksi->nama_jenis_transaksi === 'Pengeluaran' || $jenisTransaksi->nama_jenis_transaksi === 'Pemasukan')) {
                // Jika ada input sub jenis transaksi, gunakan itu
                $subJenisTransaksiId = $request->input('edit_sub_jenis_transaksi_id');
                $data['sub_jenis_transaksi_id'] = $subJenisTransaksiId ?: null; // null jika kosong
            } else {
    ...
    private function processSingleKeuanganTefa(Request $request)
    {
        ...
        $data = [
            ...
            'sub_jenis_transaksi_id' => $request->input('sub_jenis_transaksi_id') ?: null, // Handle nullable
    ...
    private function hasSubJenisTransaksiData($jenisTransaksiId, $jenisKeuanganTefaId)
    {
        return DB::table('m_sub_jenis_transaksi')
            ->where('jenis_transaksi_id', $jenisTransaksiId)
            ->where('jenis_keuangan_tefa_id', $jenisKeuanganTefaId)
            ->whereNull('deleted_at')
            ->exists();
    ...
    public function checkSubJenisAvailability(Request $request)
    {
        try {
            $jenisTransaksiId = $request->input('jenis_transaksi_id');
            $jenisKeuanganTefaId = $request->input('jenis_keuangan_tefa_id');
            ...
            $hasData = $this->hasSubJenisTransaksiData($jenisTransaksiId, $jenisKeuanganTefaId);
