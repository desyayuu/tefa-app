    ...
    <div class="card shadow-sm border-0 rounded-3">
        <div class="card-body">
            <div class="title-filter d-flex justify-content-between align-items-center mb-3">
                <h5 class="m-0">Partisipasi Mahasiswa dalam Proyek</h5>
                <div class="d-flex gap-2 align-items-center">
                    <div class="position-relative">
                        <form action="" method="GET">
                            ...
                            <input type="text" name="search_partisipasi" class="form-control pe-5 form-search" 
                                   placeholder="Cari partisipasi maahasiswa..." value="{{ $searchPartisipasi ?? '' }}">
                    ...
                    @if(isset($searchPartisipasi) && $searchPartisipasi)
                        <a href="{{ route('koordinator.dataMahasiswa') }}{{ isset($search) && $search ? '?search=' . $search : '' }}" 
                           class="btn btn-tutup btn-outline-secondary">
                            Hapus Filter Partisipasi
                        </a>
                    @endif
                </div>
            </div>
            ...
            <div class="alert alert-info">
                Menampilkan {{ $partisipasiMahasiswa->count() }} hasil partisipasi untuk pencarian "{{ $searchPartisipasi }}"
            </div>
            @endif
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        @forelse ($partisipasiMahasiswa as $p)
                        <tr>
                            <td>{{ $p->nama_mahasiswa}}</td>
                            <td>{{ $p->nama_proyek }}</td>
                            <td>
                                <span class="badge {{ $p->role_type == 'Leader' ? 'bg-warning' : 'bg-secondary' }}">
                                    {{ $p->role_type }}
                                </span>
                            </td>
                            <td>
            ...
            @if($partisipasiMahasiswa->hasPages())
            <div class="d-flex justify-content-between align-items-center mt-3">
                <div class="showing-text">
                    Showing {{ $partisipasiMahasiswa->firstItem() }} to {{ $partisipasiMahasiswa->lastItem() }} 
                    of {{ $partisipasiMahasiswa->total() }} partisipasi
                </div>
                <div class="pagination-links">
                    {{ $partisipasiMahasiswa->appends([
