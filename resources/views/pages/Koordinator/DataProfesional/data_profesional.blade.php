...
<div class="content-table">
    <div class="card-data-profesional" style="font-size: 14px;">
        <div class="card-body">
            ...
            <form id="formEditProfesional" action="{{ route('koordinator.updateDataProfesional', $profesional->profesional_id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                <label for="status_akun_profesional" class="form-label">Status Akun <span class="text-danger">*</span></label>
                                <select class="form-select @error('status_akun_profesional') is-invalid @enderror" 
                                        id="status_akun_profesional" 
                                        name="status_akun_profesional"
                                        data-original-value="{{ $profesional->status }}"
                                        required>
                                    <option value="Active" {{ old('status_akun_profesional', $profesional->status) == 'Active' ? 'selected' : '' }}>Active</option>
                                    <option value="Pending" {{ old('status_akun_profesional', $profesional->status) == 'Pending' ? 'selected' : '' }}>Pending</option>
                                    <option value="Rejected" {{ old('status_akun_profesional', $profesional->status) == 'Rejected' ? 'selected' : '' }}>Rejected</option>
                                    <option value="Disabled" {{ old('status_akun_profesional', $profesional->status) == 'Disabled' ? 'selected' : '' }}>Disabled</option>
                                </select>
                                <div class="invalid-feedback" id="status_akun_profesional_error">
                                    @error('status_akun_profesional'){{ $message }}@enderror
                                </div>
                            </div>
                        </div>
