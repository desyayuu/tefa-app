<div class="anggota-proyek-container flex-grow-1 me-3">
    <div class="card shadow-sm border-0 rounded-3 h-100">
                    ...
                    <div class="list-anggota">
                        @foreach($anggotaDosen as $anggota)
                            <div class="d-flex align-items-center justify-content-between py-2">
                                ...
                                <div>
                                    @if($isLeader)
                                    <form action="{{ route('dosen.hapusAnggotaDosen', ['proyekId' => $proyek->proyek_id, 'memberId' => $anggota->project_member_dosen_id]) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus anggota dosen ini?');">
                                        @csrf
                                        @method('DELETE')
                    ...
                    <div class="list-anggota">
                        @foreach($anggotaProfesional as $anggota)
                            <div class="d-flex align-items-center justify-content-between py-2">
                                ...
                                <div>
                                    @if($isLeader)
                                    <form action="{{ route('dosen.hapusAnggotaProfesional', ['proyekId' => $proyek->proyek_id, 'memberId' => $anggota->project_member_profesional_id]) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus anggota profesional ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm border-0 bg-transparent p-0">
                                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="list-anggota">
                        @foreach($anggotaMahasiswa as $anggota)
                            <div class="d-flex align-items-center justify-content-between py-2">
                                ...
                                <div>
                                    @if($isLeader)
                                        <form action="{{ route('dosen.hapusAnggotaMahasiswa', ['proyekId' => $proyek->proyek_id, 'memberId' => $anggota->project_member_mahasiswa_id]) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus anggota mahasiswa ini?');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm border-0 bg-transparent p-0">
                                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        @include('pages.Dosen.DataProyek.modal_anggota_proyek')
    </div>
</div>