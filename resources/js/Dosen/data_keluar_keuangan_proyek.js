...
function initializeViewProjectButtons() {
    ...
    viewButtons.forEach(button => {
        button.addEventListener('click', function(e) {
            ...
            window.location.href = `/dosen/data-keluar-keuangan-proyek/${proyekId}`;
        });
    });
...
function loadTransactionDetailForEdit(transaksiId) {
    ...
    fetch(`/dosen/data-keluar-keuangan-proyek/${proyekId}/transaksi/${transaksiId}/detail`, {
        method: 'GET',
        headers: {
...
function deleteTransaction(transaksiId) {
    ...
    fetch(`/dosen/data-keluar-keuangan-proyek/hapus-transaksi/${transaksiId}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': csrfToken,
            'Accept': 'application/json',
...
function submitForm() {
    ...
    if (!isSingle && itemList.length > 0) {
        ...
        $.ajax({
            url: '/dosen/data-keluar-keuangan-proyek/store-with-files',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
        ...
        $.ajax({
            url: '/dosen/data-keluar-keuangan-proyek/tambah-transaksi',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
...
function submitEditForm() {
    ...
    $.ajax({
        url: `/dosen/data-keluar-keuangan-proyek/update-transaksi/${transaksiId}`,
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
...
function loadTransaksiData(page = 1, filters = {}) {
    ...
    fetch(`/dosen/data-keluar-keuangan-proyek/${proyekId}/transaksi?${params.toString()}`, {
        method: 'GET',
        headers: {
            'X-CSRF-TOKEN': csrfToken || '',
            'Accept': 'application/json',
...
function loadKategoriPengeluaranForFilter() {
    ...
    const proyekId = proyekIdInput.value;
    const requestUrl = '/dosen/data-keluar-keuangan-proyek/get-kategori-pengeluaran';
...
function updateFinancialSummary() {
    ...
    $.ajax({
        url: `/dosen/data-keluar-keuangan-proyek/${proyekId}/summary`,
        type: 'GET',
        dataType: 'json',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content'),
