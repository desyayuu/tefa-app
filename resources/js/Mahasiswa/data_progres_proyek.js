...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadMyProgresProyek(page = 1) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success && response.data) {
                    // ✅ TAMBAHAN: Update nama mahasiswa di modal jika ada
                    if (response.mahasiswaInfo && response.mahasiswaInfo.nama_mahasiswa) {
                        ...
                        console.log('Updated mahasiswa name:', response.mahasiswaInfo.nama_mahasiswa); // untuk debug
                    }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    const isCreatedByCurrentUser = response.isCreatedByCurrentUser || false; // ✅ NEW
    ...
    function applyFieldRestrictions(isLeader, canEdit, editableFields, isCreatedByCurrentUser = false) {
        ...
        if (!canEdit && !isLeader) {
            ...
            // ===============================
            // EDIT MODE FOR MAHASISWA (Member)
            // ===============================
            ...
            if (isCreatedByCurrentUser) {
                ...
                // Mahasiswa yang hanya ditugaskan (bukan pembuat)
                $('#edit_nama_progres').closest('.mb-3').before(`
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.mahasiswaInfo && response.mahasiswaInfo.nama_mahasiswa) {
                    $('#my_assignment_display').val(response.mahasiswaInfo.nama_mahasiswa);
                    console.log('Loaded mahasiswa name via AJAX:', response.mahasiswaInfo.nama_mahasiswa);
                }
            },
            error: function(xhr) {
    ...
    function loadTeamMembersForEdit(callback) {
        ...
        $.ajax({
            url: `/mahasiswa/progres-proyek/${proyekId}/team-members`,
            method: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                        ...
                        if (typeof $.fn.select2 !== 'undefined') {
                            try {
                                $('.select2-assign-leader').select2({
                                    theme: 'bootstrap-5',
                                    dropdownParent: $('#modalTambahProgresFromMahasiswa'),
                                    placeholder: 'Pilih Project Leader',
                                    width: '100%'
                                });
    ...
    $('#modalTambahProgresFromMahasiswa').on('shown.bs.modal', function () {
    ...
    $('#btnTambahkanKeDaftarProgres').on('click', function() {
        ...
        const isMyProgres = (modalId === 'modalTambahProgresFromMahasiswaSelf');
    ...
    $('#modalTambahProgresFromMahasiswa').on('hidden.bs.modal', function () {
        $('#formTambahDataProgres')[0].reset();
        $('.invalid-feedback').text('');
        $('#form_progres_error').addClass('d-none').text('');
        progressList = [];
