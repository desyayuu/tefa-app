...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function loadDataProgresProyek(page = 1) {
        ...
        $.ajax({
            url: `/profesional/progres-proyek/${proyekId}/get`,
            type: 'GET',
            data: {
                search: searchParam,
                page: page, 
    ...
    function loadMyProgresProyek(page = 1) {
        ...
        $.ajax({
            url: `/profesional/progres-proyek/${proyekId}/my-progres/get`,
            type: 'GET',
            data: {
            ...
            success: function(response) {
                ...
                if (response.success && response.data) {
                    // ✅ TAMBAHAN: Update nama profesional di modal jika ada
                    if (response.profesionalInfo && response.profesionalInfo.nama_profesional) {
                        $('#my_assignment_display').val(response.profesionalInfo.nama_profesional);
                        console.log('Updated profesional name:', response.profesionalInfo.nama_profesional); // untuk debug
                    }
    ...
    function loadProgresProyekDetail(id) {
        ...
        $.ajax({
            url: `/profesional/progres-proyek/${id}/detail`,
            type: 'GET',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
    ...
    function deleteProgresProyek(progresProyekId) {
        $.ajax({
            url: `/profesional/progres-proyek/${progresProyekId}/delete`,
            type: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
    ...
    $('#modalTambahProgresFromProfesionalSelf').on('shown.bs.modal', function () {
        // Reset form
    ...
    function loadCurrentProfesionalName() {
        ...
        $.ajax({
            ...
            data: {
                page: 1,
                per_page_my_progres: 1  // hanya ambil 1 data untuk get profesionalInfo
            },
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function(response) {
                if (response.success && response.profesionalInfo && response.profesionalInfo.nama_profesional) {
                    $('#my_assignment_display').val(response.profesionalInfo.nama_profesional);
                    console.log('Loaded profesional name via AJAX:', response.profesionalInfo.nama_profesional);
                }
            },
            error: function(xhr) {
    ...
    function loadTeamMembersForEdit(callback) {
        ...
        $.ajax({
            url: `/profesional/progres-proyek/${proyekId}/team-members`,
            method: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.success) {
    ...
    $('#formEditProgres').on('submit', function(e) {
        ...
        $.ajax({
            url: `/profesional/progres-proyek/${progresId}/update`,
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $('#btnTambahkanKeDaftarProgres').on('click', function() {
        ...
        const isMyProgres = (modalId === 'modalTambahProgresFromProfesionalSelf');
    ...
    $('#formTambahDataProgres').on('submit', function(e) {
        ...
        // Determine endpoint
        const endpoint = isMyProgres ? '/profesional/progres-proyek/my-progres/store' : '/profesional/progres-proyek/store';
    ...
    $('#modalTambahProgresFromProfesionalSelf').on('hidden.bs.modal', function () {
        $('#formTambahDataProgres')[0].reset();
        $('.invalid-feedback').text('');
        $('#form_progres_error').addClass('d-none').text('');
        myProgressList = [];
